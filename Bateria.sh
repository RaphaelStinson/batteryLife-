...
if [ -f "$caminho_arquivo_batt_discharge" ]; then
    ...
    # Verifica se o valor é numérico
    if echo "$valor_batt_discharge" | grep -E '^[0-9]+(\.[0-9]+)?$' > /dev/null; then
        # Divide o valor por 100 usando divisão inteira
        valor_dividido=$((valor_batt_discharge / 100))
        
        # Exibe o valor dividido com cores
        echo -e "${BLUE}The total number of cycles is: ${GREEN}${valor_dividido}${RESET}"
    else
        echo -e "${RED}O valor '$valor_batt_discharge' no arquivo '${caminho_arquivo_batt_discharge}' não é um número válido.${RESET}"
    fi
else
